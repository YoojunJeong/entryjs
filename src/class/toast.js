...
Entry.Toast = function () {
    this.toasts_ = [];
    /** @type {Element} */
    let exist = document.getElementById('entryToastContainer');
...
Entry.Toast.prototype.warning = function (title, message, isNotAutoDispose) {
    if (window.android && window.android.failUpload) {
        window.android.failUpload(message);
    } else {
...
Entry.Toast.prototype.success = function (title, message, isNotAutoDispose) {
    let toast = Entry.createElement('div', 'entryToast');
    toast.addClass('entryToast');
    ...
    let toastTitle = Entry.createElement('div', 'entryToast');
    toastTitle.addClass('entryToastTitle');
    toastTitle.innerHTML = title;
    toast.appendChild(toastTitle);
    let toastMessage = Entry.createElement('p', 'entryToast');
    toastMessage.addClass('entryToastMessage');
    toastMessage.innerHTML = message;
    toast.appendChild(toastMessage);
    this.toasts_.push(toast);
